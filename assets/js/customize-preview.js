/* global mhbasicthemeBgColors, mhbasicthemePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
	...
	api( 'menu_background_color', function( value ) {
		value.bind( function( to ) {
			$( '#headerWrapper .bg-wrapper' ).css( 'background-color', to );
		} );
	} );
	...
	api( 'header_background_color', function( value ) {
		value.bind( function( to ) {
			...
			$( 'body:not(.overlay-header)#headerWrapper' ).css( 'background-color', to );
	...
	api( 'footer_background_color', function( value ) {
		value.bind( function( to ) {
			// Add background color to header and footer wrappers.
			$( '#siteFooter' ).css( 'background-color', to );
	...
	api( 'background_color', function( value ) {
		value.bind( function( to ) {
			...
			if ( to.toLowerCase() === api( 'header_background_color' ).get().toLowerCase() ) {
				$( 'body' ).addClass( 'reduced-spacing' );
			} else {
				$( 'body' ).removeClass( 'reduced-spacing' );
			}
	...
	api( 'accent_hue', function( value ) {
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( mhbasicthemeBgColors ).forEach( function( context ) {
					mhbasicthemeGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	...
	Object.keys( mhbasicthemeBgColors ).forEach( function( context ) {
		wp.customize( mhbasicthemeBgColors[ context ].setting, function( value ) {
			value.bind( function() {
				// Generate the styles.
				// Add a small delay to be sure the accessible colors were generated.
				setTimeout( function() {
	 ...
	 * @return {void}
	 */
	function mhbasicthemeGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'mhbasictheme-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#mhbasictheme-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( mhbasicthemePreviewEls[ context ] ) {
				_.each( mhbasicthemePreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		mhbasicthemeGenerateColorA11yPreviewStyles( 'content' );
		mhbasicthemeGenerateColorA11yPreviewStyles( 'header' );
		mhbasicthemeGenerateColorA11yPreviewStyles( 'footer' );
		mhbasicthemeGenerateColorA11yPreviewStyles( 'menu' );
	} );
