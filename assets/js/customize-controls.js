/* global mhbasicthemeBgColors, mhbasicthemeColor, jQuery, wp, _ */
/**
...
( function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( mhbasicthemeBgColors ).forEach( function( context ) {
					...
					if ( mhbasicthemeBgColors[ context ].color ) {
						backgroundColorValue = mhbasicthemeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( mhbasicthemeBgColors[ context ].setting ).get();
					}
					mhbasicthemeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( mhbasicthemeBgColors ).forEach( function( context ) {
			wp.customize( mhbasicthemeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					...
					mhbasicthemeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function mhbasicthemeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = mhbasicthemeColor( backgroundColor, accentHue );
